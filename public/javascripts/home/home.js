...
$(document).ready(function () {
  ...
  $('#dropdownAreas').html(Object.keys(AREAS).map(
    function (area) {
      return '<a href="#">' + area + '</a>'
    }).join(''));

  var map = L.map('leafletMap', {center: DEFAULT_COORDS, zoom: DEFAULT_ZOOM});
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    attribution: '&copy; OpenStreetMap contributors'
  }).addTo(map);

  var TRAFFIC_URL = 'https://opendata.arcgis.com/datasets/6ba5258ffea34e878168ddc8cf34f7e3_250.geojson';
  $.get(TRAFFIC_URL, function (trafficJson) {
    var allTrafficMarkers = getTrafficMarkersFromFeatures(trafficJson.features);
    var leafletMarkerDict = getLeafletMarkerDict(trafficJson.features);
